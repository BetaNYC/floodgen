...
import { WindowContext, WindowContextType } from '@/contexts/windowContext'
...
const InfoBox = () => {
    ...
    const { desktop, width, setWidth } = useContext(WindowContext) as WindowContextType
        ...
        <>
            <div className={`absolute top-[36%]  lg:top-0 w-full h-[64%] lg:h-full bg-white lg:bg-secondary_blue rounded-t-[1rem] lg:rounded-none z-30 overflow-y-auto ${boxShown ? "translate-y-0 duration-700" : "translate-y-full duration-700"}`}>
                <div className=' px-[1.125rem] lg:px-[4rem] pt-[3rem] lg:pt-[5rem] pb-[2rem] w-full rounded-t-[1rem]'>
                    <Image width={width > desktop ? 203 : 80} height={width > desktop ? 38.17 : 15.4} src={`${width > desktop ? "./logos/floodgen.svg" : "./logos/floodgen.svg"}`} alt='floodgen' />
                    {/* <InfoTitle /> */}
